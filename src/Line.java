...
public class Line extends Frame{
	...
	public static class Handler extends JComponent implements MouseInputListener, MouseWheelListener {
		...
		protected void paintComponent(Graphics g) { //CREATING SCALABLE COORDINATE SYSTEM
			...
			if (algoUsed=="DDA") {
				if (lineStyle=="dashed") {
					ddaDashedLine(ga, lineThickness);
				} else if (lineStyle=="solid"){
					ddaSolidLine(ga, lineThickness);	
				} else {
					ddaDottedLine(ga, lineThickness);
				}
			} else {
				if (lineStyle=="dashed") {
					brDashedLine(ga, lineThickness);
				} else if (lineStyle=="solid"){
					brSolidLine(ga, lineThickness);	
				} else {
					brDottedLine(ga, lineThickness);
				}
			}
		}

        public void brDottedLine(Graphics2D ga, int thick) {
            ...
            if ((m > 0) && (m < 1)) {
                ...
                step = Math.abs(dx);
                for (int i = 0; i < step; i = i + (2 * thick)) {
                    for (int on = 0; on < thick; on++) {
                        if (pk < 0) {
                            x = x + 1;
                            pk = pk + dy2;
                        } else {
        ...
        public void brDashedLine(Graphics2D ga, int thick) {
            int dashlen = 15;
            ...
            if ((m > 0) && (m < 1)) {
                step = Math.abs(dx);
                for (int i = 0; i < step; i = i + (2 * thick) + dashlen) {
    
                    for (int on = 0; on < thick + dashlen; on++) {
                        if (pk < 0) {
                            x = x + 1;
                            pk = pk + dy2;
                        } else {
        ...
        public void brSolidLine(Graphics2D ga, int thick) {
        ...
        public void ddaDashedLine(Graphics2D ga, int thick) {
            ...
            for (int i = 1; i <= (int) steps; i = i + (2 * (thick) + 12)) {
                for (int t = 0; t < thick + 10; t++) {
        ...
        public void ddaSolidLine(Graphics2D ga, int thick) {
            float xB, yB, x, y, dx, dy, xincr, yincr, steps;
            int prevX, prevY, X, Y;
        ...
        public void ddaDottedLine(Graphics2D ga, int thick) {
            ...
            if (Math.abs(dy) <= Math.abs(dx)) {
                ...
                for (int i = 0; i <= step; i = i + (2 * thick)) {
                    for (int dotLength = 0; dotLength < thick; dotLength++) {
    
                        for (int j = 1; j <= thick / 2; j++) { // left hand
                    ...
                    for (int gapLength = 0; gapLength < thick; gapLength++) {
                        x = x + xIncr;
                        y = y + yIncr;
                    }
                }
